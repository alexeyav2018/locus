...
- [x] 1.1 В `src/test/java/ru/locus/mastery/` завести тест (например,
...
- [x] 1.2 В том же тесте — попытка `ProblemService.edit(...)` с тем же
      ...
      в Задания»).
- [x] 1.3 В том же тесте — после отказа отметка (`MasteryRepository.findByStudent`)
      и разметка Задачи (`ProblemService.problem(id).methods()`) не изменились.
      Проверить прогоном того же теста.
- [x] 1.4 Добавить в Javadoc класса ссылку на ADR-0011 («Последствия»),
...
- [x] 2.1 В `openspec/backlog.md`, строка 18 (`method-edit-impact`) — пометить
...
- [x] 2.2 `openspec/context/adr/0011-otmetka-na-pare.md` не правится: запись
      неизменяема, новое решение не принимается. Проверить: файл не изменён
      (`git diff` по нему пуст).
- [x] 2.3 `CLAUDE.md` не правится: элемент не входит в перечисленные
