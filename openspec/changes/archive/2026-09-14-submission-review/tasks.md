...
- [x] 1.1 Завести миграцию `src/main/resources/db/changelog/migrations/0008-submission-review.yaml`,
      ...
      зелёные.
- [x] 1.2 В `application.yaml` — `spring.servlet.multipart.max-file-size: 20MB`
...
- [x] 2.1 Завести `StudentWorkId` и `StudentWorkFileId` по образцу
...
- [x] 2.2 Завести запись `StudentWorkFile(StudentWorkFileId id, FileKey key,
...
- [x] 3.1 Завести `StudentWorkRepository` на `JdbcClient`, `UserId` в каждом
      ...
      владельца.
- [x] 3.2 Тест `OwnerIsRequiredByWorksTest` по образцу
...
- [x] 4.1 Завести `WorksOfAssignment implements AssignmentWork` (`@Component`):
      ...
      по-прежнему зелёные.
- [x] 4.2 Поправить пояснения там, где сказано «ответчиков нет»:
...
- [x] 5.1 Завести `StudentWorkService` с `@PreAuthorize("hasRole('TEACHER')")`
      ...
      Учителя.
- [x] 5.2 Тест файлов `WorkFilesTest` на локальном хранилище по образцу
...
- [x] 6.1 Добавить `Addresses.WORKS = "/works"`. `StudentWorkController`:
...
- [x] 6.2 Шаблоны `templates/work/assignment.html` (экран приёма: заголовок
      ...
      зелёные.
- [x] 6.3 В тестовом `Browser` — перегрузка `postMultipart(String action,
      ...
      тест зелёный.
- [x] 6.4 Тест доступа `StudentWorkAccessTest` по образцу
...
- [x] 7.1 Тест `WorksAreFilteredByOwnerTest` через HTTP двумя Учителями
...
- [x] 8.2 Проверить признак готовности карточки: скан невозможно получить
      ...
      и зелёные.
- [x] 8.3 Правка документов контекста: `glossary.md` — перечисление
...
- [x] 8.4 Перевести [ADR-0038](../../context/adr/0038-rabota-odna-na-paru-i-udalyaetsya-svobodno.md)
