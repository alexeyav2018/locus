...
- [x] 4.1 `MasteryAccessTest` дополнить: `GET /mastery?student=` —
      Администратору без роли Учителя отклонён, Учителю доступен,
      без входа — форма входа. Проверка: тест зелёный.
- [x] 4.2 `MasteryIsFilteredByOwnerTest` дополнить через HTTP двумя
...
- [x] 5.1 Прогнать `mvn clean package`. Проверка: сборка зелёная.
- [x] 5.2 Проверить признак готовности карточки: единого значения нет —
...
- [x] 5.3 Правка документов контекста: `glossary.md` — у `Владение`
...
- [x] 5.4 `strategy.md` не правится, но достижение горизонта редакции 1
