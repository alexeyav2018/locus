...
- [x] 3.1 Завести `TheoryMaterialId` по образцу `ProblemId`. Проверка:
      `TheoryMaterialIdTest` — отрицательное и нулевое значение отвергаются.
- [x] 3.2 Завести запись `TheoryMaterial` (идентификатор, название, узел, `FileKey`,
...
- [x] 3.3 Завести `TheoryMaterialRepository` на `JdbcClient` по образцу
      ...
      на Testcontainers.
- [x] 3.4 Тест изоляции `OwnerIsUnknownToTheoryTest` по образцу
...
- [x] 4.1 Завести `TheoryService`: заведение, правка названия и узла, замена
      ...
      исчезновение прежнего файла при замене и при удалении. Внести `TheoryService`
      в список классов `OwnerIsUnknownToTheoryTest` (задача 3.4 завела его
      по классам, существовавшим на тот момент; каждый новый класс области
      вносится туда же).
- [x] 4.2 Добавить `TheoryService.materialsOn(TaxonomyNodeId)`: материалы узла
...
- [x] 4.3 Добавить выдачу временной подписанной ссылки на файл материала.
