...
public class SolutionMethodService {
     ...
     *
     * <p><b>Обе сущности, ссылающиеся на Метод, пополнили эту проверку</b>:
     *
     * <ul>
     *   <li>разметка Задачи — связь {@code problem_solution_method};
     *       пришла с работой {@code problem-catalog}, отвечает
     *       {@code ProblemMarkupUsage};</li>
     *   <li>отметки Владения — на паре «Тема × Метод», их ячейки опираются
     *       на удаляемый Метод; пришли с работой {@code mastery-marks},
     *       отвечает {@code MasteryOfMethod}, считая отметки всех Учителей
     *       (ADR-0036).</li>
     * </ul>
     *
     * <p>Почему это важно: удаление употреблённого Метода стёрло бы основание
     * ячеек владения у всех учителей сразу, то есть их суждений об учениках,
     * которых удаляющий не видит, и восстановить их было бы неоткуда
     * (ADR-0010, antipatterns.md, «Молчаливое удаление Темы»). Ответ
     * отвечает первый попавшийся: причин может быть две, называется одна,
     * и после её устранения отказ назовёт вторую.
     *
